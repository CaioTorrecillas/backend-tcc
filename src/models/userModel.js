...
createUser = async (user) => {
    ...
    const newUser = await connection.query("insert into clientes (name, endereco, tipoUsuario, ativo, telefone) values (?,?,?,?,?) ", values);  
    return {id: newUser};
}

deleteUser = async(id) => {
   
    const removedUser = await connection.query("delete from clientes where id=?", id);
    return removedUser;   
...
updateUser = async(id, user) => {
    ...
    return updatedUser = await connection.query("update clientes set name=?, endereco=?, tipoUsuario=?, ativo=?, telefone=? where id=?", values);     
    return updatedUser;
}
